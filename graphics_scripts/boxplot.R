...
somatic_recall <- ggplot(somatic, aes(x = SNV.Indel, y = Recall)) +            # Applying ggplot function
  geom_boxplot() + ylim(0.4,1) + ggtitle("Recall (somatic variant calling)") +
...
somatic_precision <- ggplot(somatic, aes(x = SNV.Indel, y = Precision)) +            # Applying ggplot function
  geom_boxplot() + ylim(0.4,1) + ggtitle(("Precsion (somatic variant calling)")) +
  xlab("Variant type") +geom_hline(yintercept=1, linetype="dashed", color = "red")

somatic_F <- ggplot(somatic, aes(x = SNV.Indel, y = F.score)) +            # Applying ggplot function
  geom_boxplot() + ylim(0.4,1) + ggtitle(("Overall indel F-score (somatic variant calling)")) +
  xlab("Variant type") +geom_hline(yintercept=1, linetype="dashed", color = "red")
...
a <- multiplot(somatic_recall, somatic_precision, somatic_F, cols=2) + title("Precision and recall against DNA control NA12878")
